...
public class blocksDisplayGUI implements GUI {

  Inventory inventory;
  private final RespawnBlocks instance;

  public blocksDisplayGUI(RespawnBlocks instance) {
    this.instance = instance;
  }

  @Override
  public void onClick(
      Player player, Inventory inventory, ItemStack clickedItem, ClickType clickType, int slot) {
  ...
  public Inventory getInventory() {
    inventory = new InventoryBuilder("&a&nBlocks", 54, this).build();
    for (int i = 0; i < instance.getRBManager().getRespawnBlocksList().size(); i++) {
      inventory.setItem(
          i,
          new ItemCreator(
                  instance.getRBManager().getRespawnBlocksList().get(i).getInitialBlockType(), 1)
              .addGlow(true)
              .addLoreLine("&cClick here to teleport to this Block!")
              .addLoreLine(
                  "&6X-Coordinate: "
                      + instance.getRBManager().getRespawnBlocksList().get(i).getxCoord())
              .addLoreLine(
                  "&6Y-Coordinate: "
                      + instance.getRBManager().getRespawnBlocksList().get(i).getyCoord())
              .addLoreLine(
                  "&6Z-Coordinate: "
                      + instance.getRBManager().getRespawnBlocksList().get(i).getzCoord())
              .setPDCInteger(
                  new NamespacedKey(instance, "xcoord"),
                  instance.getRBManager().getRespawnBlocksList().get(i).getxCoord())
              .setPDCInteger(
                  new NamespacedKey(instance, "ycoord"),
                  instance.getRBManager().getRespawnBlocksList().get(i).getyCoord())
              .setPDCInteger(
                  new NamespacedKey(instance, "zcoord"),
                  instance.getRBManager().getRespawnBlocksList().get(i).getzCoord())
              .getItemStack());
    }
    return inventory;
